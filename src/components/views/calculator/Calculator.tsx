...
import useHotkeyDispatch, {
  hotkeyOptions,
} from "../../../hooks/useHotkeyDispatch";
...
const selectInputForm = (
  inputForm: InputForm,
  overwriteDocumentInputMode: boolean
) => {
  if (overwriteDocumentInputMode) return <DocumentInputForm />;
...
const Calculator: React.FC = () => {
  ...
  const [currentInput, currentOutput] = useSelector((state: RootState) => [
  ...
  return <>{selectInputForm(inputForm, session.temporaryEnableDocumentMode)}</>;
};
