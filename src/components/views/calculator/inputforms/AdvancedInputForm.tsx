...
const MathHistoryItemContainer = styled.div<{ showCellNumbers: boolean }>`
  ...
  grid-template-columns: ${(props) =>
    props.showCellNumbers ? "80px auto 1fr" : "auto 1fr"};
...
const MathHistoryItemValue = styled.span<{ copyable: boolean }>`
  padding: 10px;
  ${(props) =>
    props.copyable ? "&:hover { background-color: rgba(0, 0, 0, 0.1); }" : null}
`;
...
const MathHistoryItem: React.FC<MathHistoryItemProps> = ({
  index,
  input,
  output,
}) => {
  const [t] = useTranslation();
  const copyOnClick = useSelector(
    (state: RootState) =>
      state.settings.interfaceSettings.copyCellContentOnClick
  );
  const showCellNumbers = useSelector(
    (state: RootState) => state.settings.interfaceSettings.showLineNumbers
  );
  const language = useSelector(
    (state: RootState) => state.settings.interfaceSettings.language
  );
  const cellLanguageTag = useSelector(
    (state: RootState) => state.settings.interfaceSettings.cellLanguageTag
  );
  const dispatch = useDispatch();

  const languageTag =
    cellLanguageTag === "inherit" ? language : cellLanguageTag;

  const onClick = (type: "input" | "output") => {
    if (copyOnClick)
      dispatch(changeCurrentInput(type === "input" ? input : output));
  };

  return (
    <MathHistoryItemContainer showCellNumbers={showCellNumbers}>
      {showCellNumbers ? (
        <MathHistoryItemLineCounter>#{index + 1}</MathHistoryItemLineCounter>
      ) : null}
...
const MathHistory: React.FC<{ items: MathHistoryItemProps[] }> = ({
  items,
}) => {
...
const AdvancedInputForm: React.FC = () => {
  const mathHistory = getMathCells(
  ...
  return (
    <InputFormWithSidebar>
      <Container>
        <MathHistory
          items={mathHistory.map((item, i) => ({ ...item, index: i }))}
        />
