...
const sessionReducer = createReducer<Session, SessionAction>(defaultState, {
  CHANGE_CURRENT_INPUT: (state, action) => ({
    ...state,
  ...
  PUSH_MATH_CELL: (state, action: PushMathCell) => {
    ...
    return {
      ...
      document: {
        ...state.document,
        cells: state.document.cells.concat([
          {
            type: "MATH",
            uuid: uuid(),
            input: "",
  ...
  UPDATE_MATH_CELL: (state, action) => {
    ...
    if (selectedCell) {
      ...
      if (action.payload.input.length > 0) {
        ...
        nextState.document.cells = nextState.document.cells.filter(
  ...
  REFRESH_MATH_CELLS: (state, action) => {
    clearScope();
    const updatedCells = state.document.cells.map(
      (cell: Cell): Cell =>
        cell.type === "MATH"
          ? {
              type: "MATH",
  ...
  DELETE_CELL: (state, action) => {
    const nextState = { ...state };
    nextState.document.cells = state.document.cells.filter(
  ...
  MOVE_CELL_SELECTION_DOWN: (state) => {
    return {
      ...state,
      selectedCell: Math.min(
        state.selectedCell + 1,
        state.document.cells.length - 1
      ),
    };
  },
