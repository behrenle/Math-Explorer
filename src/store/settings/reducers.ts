...
const settingsReducer = (
  ...
  switch (action.type) {
    case "SET_SETTINGS":
      // @ts-ignore
      document.getElementsByTagName("html").item(0).lang =
        action.payload.interfaceSettings.language;
      i18n
        .changeLanguage(action.payload.interfaceSettings.language)
        .catch(console.error);
    ...
    default:
      i18n
        .changeLanguage(state.interfaceSettings.language)
        .catch(console.error);
      return state;
  }
};
